...
class Graph {
 ...
 public:
   ...
   */
  Node add_node(const Point& someposition, const node_value_type& somevalue = node_value_type()) {
    // HW0: YOUR CODE HERE
  ...
  class Edge : private totally_ordered<Edge>{
   public:
    ...
    edge_value_type& value() {
      return graph->edge_values[edgeindex];
    }
    ...
    /* edge_type reverse() const {
      return edge_type(graph, node2(), node1()); 
    } */
   ...
   * Complexity: No more than O(num_nodes() + num_edges()), hopefully less
   */
  Edge add_edge(const Node& a, const Node& b, const edge_value_type& somevalue = edge_value_type()) {
    ...
    if (has_node(a) && has_node(b)) {
      ...
      edge_values.push_back(somevalue); 
      
      std::vector<size_type> some_edge2;
      some_edge2.push_back(b.index());
      some_edge2.push_back(a.index());
      edges.push_back(some_edge2);
      edge_values.push_back(somevalue); 

      adj_lists[a.index()].push_back(num_edges()-2);
