...
class Graph {
 ...
 public:
  ...
  class NodeIterator {
   public:
    ...
    Node operator*() const {
      ...
      return graph_->get_node_by_uid(current_uid);
    }
  ...
  class IncidentIterator {
   public:
    ...
    Edge operator*() const {
      ...
      Node adj_node = graph_->get_node_by_uid(adj_node_uid);
      int has_edge_uid = graph_-> has_edge(spawn_node_, adj_node);
      Edge edge = graph_->get_edge_by_uid(has_edge_uid);
  ...
  class EdgeIterator {
   public:
    ...
    Edge operator*() const {
      ...
      return graph_->get_edge_by_uid(current_uid);
    }
  ...
  /** Return the edge for a given uid.
   */
  Edge get_edge_by_uid(size_type uid) {
    internal_edge int_edge = internal_edges_[uid];
    return edge(int_edge.idx);
  }
  
  /** Return the node for a given uid.
   */
  Node get_node_by_uid(size_type uid) {
    internal_node int_node = internal_nodes_[uid];
    return node(int_node.idx);
  }
   ...
   *
   * @post The idx value for internal_nodes beyond the internal node
   * corresponding to @n decreases by 1 and the idx value for the internal
   * node corresponding to @n is i2u_nodes_.size() to indicate it is not
   * "registered" in the index. 
   * @post All edges incident to @n are removed from adjacency list
   * and idx values of internal_nodes are updated accordingly.
   */ 
  void remove_node(const Node& n) {
    std::vector<size_type> adj_nodes = adj_[n.uid()];
    std::vector<size_type> edges_to_remove_uids;
    std::vector<size_type> edges_to_remove_indices;

    // Track UID and indices of all edges incident to node to remove.
    for (auto it = adj_nodes.begin(); it != adj_nodes.end(); ++it) {
      size_type adj_node_uid = *it;
      Node adj_node = get_node_by_uid(adj_node_uid);
      size_type edge_to_remove_uid = has_edge(n, adj_node); 
      Edge edge_to_remove = get_edge_by_uid(edge_to_remove_uid);
      size_type edge_to_remove_idx = edge_to_remove.index();

      edges_to_remove_uids.push_back(edge_to_remove_uid);
      edges_to_remove_indices.push_back(edge_to_remove_idx);
    }

    for (size_type i = 0; i < edges_to_remove_uids.size(); ++i) {
      Edge e = get_edge_by_uid(edges_to_remove_uids[i]);
    ...
    // Update index to UID mapping to remove edges incident to node to remove.
    std::vector<size_type> temp;
    for (size_type i = 0; i < i2u_edges_.size(); ++i) {
      if (std::find(edges_to_remove_uids.begin(), edges_to_remove_uids.end(),
                i2u_edges_[i]) == edges_to_remove_uids.end()) {
        // Not found. Only add elements NOT corresponding to elements to remove.
        temp.push_back(i2u_edges_[i]);
      }
    }
    size_type former_i2u_size = i2u_edges_.size();
    i2u_edges_ = temp;

    // Update internal_edges idx values.
    size_type counter = 0;
    for (size_type i = 0; i < internal_edges_.size(); ++i) {
      internal_edge& int_edge = internal_edges_[i];
      if (std::find(edges_to_remove_uids.begin(), edges_to_remove_uids.end(),
		int_edge.edge_uid) != edges_to_remove_uids.end()) {
        // Then this internal edge is an edge to remove.
        int_edge.idx = i2u_edges_.size();
      } else {
        // Update idx values for all edges that have not been removed.
        if (int_edge.idx < former_i2u_size) {
          int_edge.idx = counter;
          counter++;
        }
      }     
    }
    ...
    for (auto it = internal_nodes_.begin() + n.uid(); // NOTE: CHANGED FROM INDEX.
        it != internal_nodes_.end(); ++it) {
      if (it == internal_nodes_.begin() + n.uid()) {
   ...
   *
   * @post The idx value for internal_edges beyond the internal edge
   * corresponding to @e decreases by 1 and the idx value for the internal
   * edge corresponding to @e is i2u_edges_.size() to indicate it is not
   * "registered" in the index. 
   */ 
  void remove_edge(const Edge& e) {
    size_type index = e.index();

    // Store number of edges before removal.
    size_type former_i2u_size = i2u_edges_.size();

    // Remove this edge uid from edge index to uid mapping vector.
