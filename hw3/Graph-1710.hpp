 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    // return node value reference for setting value 
    ...
    // return iterator to the first edge attached to this node 
    incident_iterator edge_begin() const {
        return IncidentIterator(graph_,this->id_,0);
    }
    
    // return iterator to the last edge attached to this node 
  ...
  bool has_node(const Node& n) const {
    if (n.id_<nsize_ and n.graph_==this->graph_) return true; 
    return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // check if adj_list_ and ev_adj_list_ has been initialized useful 
    // only when the first time add_edge is called after add_node is called
    if (adj_list_.size()<=nsize_) {
        adj_list_.resize(nsize_);
        ev_adj_list_.resize(nsize_);
    }
...
/* methods for removing edges
 ...
 private:
    // For the data representation here, I see how it can be 
    // improved by using only adjacent list. Some of the unnecessary 
    // data are kept to guarantee functionality of the existing codes.
