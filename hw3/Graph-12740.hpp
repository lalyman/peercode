...
class Graph {
 ...
 public:
  ...
  /** Predeclaration of node and edge value type*/
  using node_value_type = V;
  ...
  class Edge: private totally_ordered<Edge>{
   public:
    ...
    /** Return the length of the current edge*/
    double length() const {
        return graph_->edgelength_[idx_];
    }

    /** Modifiable edge value*/
    edge_value_type& value(){
      return graph_->edgevalues_[idx_];
    }
    ...
    bool operator==(const Edge& e) const {
      return ((n1_[idx_]==e.n1_[idx_] && n2_[idx_]==e.n2_[idx_]) || (n2_[idx_]==e.n1_[idx_] && n1_[idx_]==e.n2_[idx_]));
    }
  ...
  Edge add_edge(const Node& a, const Node& b, const edge_value_type& value = edge_value_type()) {
    ...
    edgevalues_.push_back(value);
    nodemap_[a.index()].push_back(num_edges()-1);
    nodemap_[b.index()].push_back(num_edges()-1);
    edgelength_.push_back(norm_2(a.position()-b.position()));
  ...
  class IncidentIterator: private totally_ordered<IncidentIterator> {
   public:
    ...
    Edge operator*() const{
      ...
      for(auto e: graph_->nodemap_.at(node_)){
        if (graph_->n1_[e] == node2 || graph_->n2_[e] == node2){
