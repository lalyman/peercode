...
class Graph {
  ...
  **/
   ...
   **/
    ...
    **/
    ...
    /**
    * @brief Return this node's reference position.
    *
    * @param none
    * @return A Point reference of the node with position.
    *
    * @pre Node object exists with valid position
    * @post A Point reference is returned
    **/
    Point& position() {
      return fetch_node().node_pt;
    }
    ...
    **/
    size_type degree() const {
      Node n = *this;
      return graph_->edge_search.find(graph_->i2u_n[n.index()])->second.size();
    }
    ...
    **/
    incident_iterator edge_begin() const {
      //Note that we now check to make sure that the edge search starts with a
      //valid beginning. This means iterating over the found value that
      //corresponds to the inner map
      hash_map::iterator it;

      if(graph_->edge_search.find(uid_) != graph_->edge_search.end()) {
        it = graph_->edge_search.find(uid_)->second.begin();
      }

      return IncidentIterator(graph_, it, uid_);
    }
    ...
    **/
    incident_iterator edge_end() const {
      ...
      if(graph_->edge_search.find(uid_) != graph_->edge_search.end()) {
        it = graph_->edge_search.find(uid_)->second.end();
      }
  ...
  **/
  ...
  Node add_node(const Point& position,
    ...
    //HW2: We now make this compatible with the removing of nodes
    newNode.idx_ = this->num_nodes();
    i2u_n.push_back(graph_nodes.size());
    graph_nodes.push_back(newNode);

    edge_search[i2u_n[newNode.idx_]];
  ...
  bool has_node(const Node& n) const {
    ...
    //the graph
    if(n.index() < this->num_nodes() && n.graph_ == this)
      return true;
  ...
  class Edge : private totally_ordered<Edge> {
    ...
    **/
    ...
    /**
    * @brief Return the reference to the Edge's value
    *
    * @param none
    * @return A edge_value_type value in the Edge object
    *
    * @pre Edge object exists with valid edge_value_type value
    * @post A edge_value_type value associated with the node is returned
    *
    * This function acts as a sort of a setter since the value is not const.
    **/
    ...
    /**
    * @brief Return a constant reference to the Edge's constant value
    *
    * @param Edge
    * @return A const node_value_type value in the Edge object
    *
    * @pre Edge object exists with valid edge_value_type value
    * @post A const edge_value_type value associated with the Edge is returned
    *
    * This function acts as a sort of a getter since the value is const.
    **/
    ...
    /**
    * @brief Return length of the edge
    *
    * @return A double of the length of the edge
    *
    * @pre Edge object exists
    * @post result is a double of the length of the edge
    **/
    double length() const {
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    auto search = edge_search.find(i2u_n[a.index()]);
    if(search != edge_search.end()) {
      //
      auto secondSearch = (search->second).find(i2u_n[b.index()]);
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if(has_edge(a, b)) {
      return Edge(this, edge_search[i2u_n[a.index()]][i2u_n[b.index()]], true);
    }
    if(has_edge(b, a)) {
      return Edge(this, edge_search[i2u_n[a.index()]][i2u_n[b.index()]], false);
    }
    ...
    new_edge.source = i2u_n[a.index()];
    new_edge.dest = i2u_n[b.index()];

    //Much like with the node adding. We now make the edge removal compatible
    //with our new abstraction
    new_edge.idx_ = this->num_edges();
    i2u_e.push_back(graph_edges.size());
    graph_edges.push_back(new_edge);

    size_type new_index = this->num_edges() - 1;
    edge_search[i2u_n[a.index()]][i2u_n[b.index()]] = new_index;
    edge_search[i2u_n[b.index()]][i2u_n[a.index()]] = new_index;
   ...
   *
   * @pre Graph object exists and node @a n exists
   * @post num_nodes() == old num_nodes()-1
   * @post node @a n is no longer in the graph_nodes nor the graph
   * @post result is number of nodes removed
   */
  size_type remove_node(const Node& n) {
    ...
    //Iterate over all the edges connected to the node. Put them in a vector
    //toRemove and them remove them. This is so that the graph doesn't update
    ...
    //Once we have removed all the edges, remove all the node as well.
    size_type uid = i2u_n[n.index()];
    auto it = i2u_n.begin() + graph_nodes[uid].idx_;
    i2u_n.erase(it);
    //This is to keep the mapping between the uid and indices consistent.
    for(it = i2u_n.begin() + graph_nodes[uid].idx_; it != i2u_n.end(); ++it) {
      graph_nodes[*it].idx_ = graph_nodes[*it].idx_ - 1;
    }
   ...
   * @brief Remove the node from the graph
   *
   * @param ni the node iterator pointing to the node to remove
   * @return the node iterator with the updated nodes
   *
   * @pre Graph object exists and node iterator @a ni exists
   * @post num_nodes() == old num__nodes()-1
   * @post node @a *ni is no longer in the graph_nodes nor the graph
   * @post result is the node iterator with the updated nodes
   */
   ...
   *
   * @param[in] a the source node of edge
   * @param[in] b the dest node of edge
   * @return the number of edges removed
   *
   * @pre Graph object exists and edge exists
   * @post num_edges() == old num_edges()-1
   * @post edge is no longer in the graph_edges nor the graph
   * @post result is number of edges removed
   */
  size_type remove_edge(const Node& a, const Node& b) {
    ...
    ///Find the edge and perform the same operation as in remove_node
    size_type uid = edge_search[i2u_n[a.index()]][i2u_n[b.index()]];

    auto it = i2u_e.begin() + graph_edges[uid].idx_;
    i2u_e.erase(it);
    for(it = i2u_e.begin() + graph_edges[uid].idx_; it != i2u_e.end(); ++it) {
      graph_edges[*it].idx_ = graph_edges[*it].idx_ - 1;
    }

    //We also have to remove the edge from the adjacency map
    edge_search[i2u_n[a.index()]].erase(i2u_n[b.index()]);
    edge_search[i2u_n[b.index()]].erase(i2u_n[a.index()]);
   ...
   * @return the number of edges removed
   *
   * @pre Graph object exists and edge @a e exists
   * @post num_edges() == old num_edges()-1
   * @post edge @e is no longer in the graph_edges nor the graph
   * @post result is number of edges removed
   */
   ...
   * @brief Remove the edge from the graph
   *
   * @param ei the edge iterator pointing to the edge we want to remove
   * @return the edge iterator with the updated edges
   *
   * @pre Graph object exists and edge iterator exists
   * @post num_edges() == old num_edges()-1
   * @post @a *ei is no longer in the graph_edges nor the graph
   * @post result is the edge iterator with the updated edges
   */
   ...
   * @brief Remove the edge from the graph
   *
   * @param ii the edge iterator pointing to the edge we want to remove
   * @return the incident iterator with the updated edges
   *
   * @pre Graph object exists and edge iterator exists
   * @post num_edges() == old num_edges()-1
   * @post @a *ii is no longer in the graph_edges nor the graph
   * @post result is the incident iterator with the updated edges
   */
  incident_iterator remove_edge(incident_iterator ii) {
    remove_edge(*ii);
   ...
   class NodeIterator : private totally_ordered<NodeIterator>{
     ...
     **/
     Node operator*() const {
       //HW2: updated to make sure removal is proper
       return Node(graph_, graph_->i2u_n[iterInd_]);
     }
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator> {
    ...
    **/
    Edge operator*() const {
      //HW2: updated to make sure removal is proper
      return Edge(graph_, graph_->i2u_e[iterInd_], true);
    }
 ...
 private:
  ...
  //Paradigm for removal was inspired by Lecture 9. We create a vector container
  //to store the vectors that we want to remove. We do this to provide
  //abstraction between our removal process and the user. Take a look at the
  //lecture slides to see why this is implemented.
