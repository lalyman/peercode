...
public:
  ...
  Node add_node(const Point &position,
    ...
    adjacency_matrix.push_back(std::vector < std::pair < size_type,
                               edge_value_type >> ());
    ++num_nodes_;
  ...
  public:
    ...
    edge_value_type &value() {
      for (auto it = node_1.edge_begin(); it != node_1.edge_end(); ++it)
        if ((*it).node2() == node_2)
          return node_1.graph_pointer->
              adjacency_matrix[node_1.index()][it.edge_position].second;
    ...
    bool operator==(const Edge &e) const {
      if ((this->node_1 == e.node_1 && this->node_2 == e.node_2) |
          (this->node_1 == e.node_2 && this->node_2 == e.node_1)) {
        return true;
      }
      return false;
    }
     ...
     */
    bool operator<(const Edge &e) const {
      if (std::min(this->node_1.n_id, this->node_2.n_id) <
          std::min(e.node_1.n_id, e.node_2.n_id))
        return true;
      else if (std::min(this->node_1.n_id, this->node_2.n_id) >
               std::min(e.node_1.n_id, e.node_2.n_id))
        return false;
      else if (std::max(e.node_1.n_id, e.node_2.n_id) <=
               std::max(e.node_1.n_id, e.node_2.n_id))
        return true;
      else return false;
    }
  ...
  Edge edge(size_type i) const {
    ...
    for (EdgeIterator ni = this->edge_begin(); ni != this->edge_end(); ++ni) {
      if (ni.node_position <
          ni.graph_pointer->
              adjacency_matrix[ni.node_position][ni.edge_position].first) {
        if (counter == i)
          return Edge(
              Node(const_cast<Graph *>(ni.graph_pointer), ni.node_position),
              Node(const_cast<Graph *> (ni.graph_pointer),
                   ni.graph_pointer->
                       adjacency_matrix[ni.node_position][ni.edge_position].first));
        else counter++;
      }
...
/** Remove an edge from the graph and return true if remove worked (i.e. the edge we wanted to remove
  ...
  size_type remove_edge(const Node &n1, const Node &n2) {
    for (size_type j = 0; j < adjacency_matrix[n1.index()].size(); ++j) {
      if (adjacency_matrix[n1.index()][j].first == n2.index()) {
        adjacency_matrix[n1.index()].erase(
            adjacency_matrix[n1.index()].begin() + j);
        break;
      }
    }
    for (size_type j = 0; j < adjacency_matrix[n2.index()].size(); ++j) {
      if (adjacency_matrix[n2.index()][j].first == n1.index()) {
        adjacency_matrix[n2.index()].erase(
            adjacency_matrix[n2.index()].begin() + j);
        --num_edges_;
        return 1;
      }
    }
  ...
  public:
    ...
    bool operator==(const NodeIterator &nodeiter) const {
      if (nodeiter.graph_pointer == this->graph_pointer &&
          nodeiter.n_position == this->n_position)
        return true;
      else return false;
    }
...
// private constructor
  ...
  public:
    ...
    // Supply definitions AND SPECIFICATIONS for:
    Edge operator*() const {
      return Edge(Node(const_cast<Graph *>(this->graph_pointer), this->node_id),
                  Node(const_cast<Graph *>(this->graph_pointer),
                       this->graph_pointer->adjacency_matrix[node_id][edge_position].first));
    ...
    bool operator==(const IncidentIterator &II) const {
      if (this->graph_pointer == II.graph_pointer &&
          this->node_id == II.node_id &&
          this->edge_position == II.edge_position)
        return true;
      else return false;
    }
  ...
  public:
    ...
    // Supply definitions AND SPECIFICATIONS for:
    Edge operator*() const {
      return Edge(Node(const_cast<Graph *>(graph_pointer), node_position),
                  Node(const_cast<Graph *>(graph_pointer),
                       graph_pointer->adjacency_matrix[node_position][edge_position].first));
    }

    EdgeIterator &operator++() {
      if (edge_position <
          graph_pointer->adjacency_matrix[node_position].size() - 1)
        edge_position++;
      else {
        node_position++;
        edge_position = 0;
    ...
    bool operator==(const EdgeIterator &EI) const {
      if (this->graph_pointer == EI.graph_pointer &&
          this->node_position == EI.node_position &&
          this->edge_position == EI.edge_position)
        return true;
      else return false;
    }
...
private:
  ...
  std::vector <std::vector<std::pair < size_type, edge_value_type>>>
  adjacency_matrix;
};
