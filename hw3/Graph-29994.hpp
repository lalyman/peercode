...
class Graph {
  ...
  class Node : private totally_ordered<Node>{
   public:
    ...
    Point& position() {
      ...
      //return const_cast<Point&>(graph_->nodes_[uid_].position);
    }
  ...
  */
  ...
  Node add_node(const Point& position, const node_value_type& value = node_value_type()) {
    ...
    //i2nu_.push_back(next_uid_node_);//add node index 
    nodes_[next_uid_node_].uid = next_uid_node_;
    next_uid_node_++;
    return Node(this,next_uid_node_-1);
  }
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    //return false;//HW2 temporary change
    (void) n;            // Quiet compiler warning
  ...
  class Edge : private totally_ordered<Edge>{
   public:
    ...
    //HW2
    double length() {
        return norm(graph_->node(graph_->edges_[uid_].uid1).position()-graph_->node(graph_->edges_[uid_].uid2).position());
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    // HW0: YOUR CODE HERE
    (void) a; (void) b;   // Quiet compiler warning
    if ( !(this==a.graph_ && this==b.graph_) ) std::cout << "Check has_edge : same graph?" << std::endl;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if ( has_edge(a,b) == false )
    {
        ...
        //i2eu_.push_back(next_uid_edge_);
        edges_[next_uid_edge_].uid = next_uid_edge_;
        next_uid_edge_++;
 ...
 private:
  ...
  // adj_ stores adjecent nodes uid for each node -> adj_[0] stores vectors of all incident nodes of node(0)
    ...
    //std::vector<size_type> i2nu_;
    //std::vector<size_type> i2eu_;
    //merge this two together -> adj_ and edges_ or save it into nodes_ Danah
