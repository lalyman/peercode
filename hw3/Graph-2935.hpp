...
class Graph {
 ...
 public:
   ...
   *         or
   *         n_it  a node iterator, so that n = *n_it
  ...
  size_type remove_node(const Node& n) {
    ...
    for (incident_iterator it = last.edge_begin(); it != last.edge_end(); ++it) {
      size_type j = (*it).node2().index();
      node_edges_map_[j][i] = node_edges_map_[j][num_nodes()];
      node_edges_map_[j].erase(num_nodes());
      internal_edge& e = (*it).fetch();
      if (e.node1 == num_nodes())
        e.node1 = i;
      else
        e.node2 = i;
    }
  ...
  bool has_edge(const node_type& a, const node_type& b) const {
    ...
    if (node_edges_map_.count(a.index_) == 1)
      if (node_edges_map_.at(a.index_).count(b.index_) == 1)
        return true;
    return false;
  }
   ...
   *         or
   *         e  an edge object passed by reference, so that n1 = e.node1(), n2 = e.node2()
   *         or
   *         e_it  an edge iterator to the edge we want to remove, so that e = *e_it
