...
public:
 ...
 private:
  ...
  //Stores the node values and poistions, indexed by node index
  std::unordered_map<size_type,std::pair<Point,node_value_type>>* nodes_;
  // Stores the edge list and the edge values, indexed by edge index
...
public:
  ...
  class Node : private totally_ordered<Node>{
   public:
     ...
     */
    bool operator<(const Node& n) const {
      return ((n.graph_<graph_)||((n.graph_==graph_)&&(index()< n.index())));
    }
 ...
 */
  size_type remove_edge(const size_type edge_index_todelete){
    if (num_edges()==1){
     //Indexes of the nodes concerned by deletion
     size_type node1_ix,node2_ix;
     //Deletion of the entries
     node1_ix = (*edges_list_)[edge_index_todelete].first.first;
     node2_ix = (*edges_list_)[edge_index_todelete].first.second;

    (*rev_edges_list_)[node1_ix]->erase(node2_ix);
    (*rev_edges_list_)[node2_ix]->erase(node1_ix);
      edges_list_->clear();
    }else{
    ...
    //delete old entry
    size_type node1_ix_todelete,node2_ix_todelete;
    //The nodes for deletion correspond to the edge with index edge_indec_toswap, since we swapped
    node1_ix_todelete = (*edges_list_)[edge_index_toswap].first.first;
    node2_ix_todelete = (*edges_list_)[edge_index_toswap].first.second;

    //We delete the old entry
    (*rev_edges_list_)[node1_ix_todelete]->erase(node2_ix_todelete);
    (*rev_edges_list_)[node2_ix_todelete]->erase(node1_ix_todelete);

    //Indexes of nodes for the swapped edge
    size_type node1_ix_toswap,node2_ix_toswap;
    //The nodes of the swapped edge correspond to the edge with index edge_indec_todelete, since we swapped
    node1_ix_toswap = (*edges_list_)[edge_index_todelete].first.first;
    node2_ix_toswap = (*edges_list_)[edge_index_todelete].first.second;
    //update index for swapped entry
    (*(*rev_edges_list_)[node1_ix_toswap])[node2_ix_toswap] = edge_index_todelete;
    (*(*rev_edges_list_)[node2_ix_toswap])[node1_ix_toswap] = edge_index_todelete;
