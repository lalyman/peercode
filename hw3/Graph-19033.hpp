...
//    }
  ...
  size_type remove_node(const Node& n) {
    ...
    if (has_node(n)) {
      ...
      for(std::vector<size_type>::iterator it = adj_[n.uid_].begin(); it != adj_[n.uid_].end(); ++it) {
        *(std::find( adj_[*it].begin(), adj_[*it].end(), n.uid_ )) = adj_[*it].back();
        adj_[*it].pop_back();
        adj_[n.uid_].empty();
      }
      //Erase the element in i2u_. O(num_nodes())
      *(std::find( i2u_.begin(), i2u_.end(), n.uid_ )) = i2u_.back();
      i2u_.pop_back();
      return 1; 
    }
    return 0;
  ...
  size_type remove_edge(const Node& a, const Node& b) {
    if ( has_edge(a, b) ) {
      *(std::find( ei2u_.begin(), ei2u_.end(), edge(a, b).uid_ )) = ei2u_.back();
      ei2u_.pop_back();
      return 1;
    }
    return 0;
