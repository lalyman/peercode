...
class Graph {
 ...
 public:
  ...
  size_type num_edges() const {
    ...
    //Alt method, but two tests in test_edges fail...
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    //return false if the nodes are the same
    if (a == b) {
      return false;
    }
   ...
   public:
    ...
    EdgeIterator& operator++() {
      ...
      while (uid1_ < graph_->adj_.size()) {
        //iterate through tuples in subvector
        while (neighbor_index_ < graph_->adj_[uid1_].size()) {
          size_type uid2 = std::get<0>(graph_->adj_[uid1_][neighbor_index_]);
          //assert global ordering
          if (uid1_ < uid2) {
            return *this;
          }
          //continue through subvector
          ++neighbor_index_;
        }   
        //go to next sub-vector
        neighbor_index_ = 0;
        ++uid1_;
      }
      return *this;
