...
class Graph {
 ...
 public:
  ...
  size_type remove_node(const Node& n) {
    ...
    while (n.degree() > 0) remove_edge(*n.edge_begin());
  ...
  size_type remove_edge(const Edge& e) {
    ...
    // obtain indices of nodes in current edge index
    size_type ind01 = e.node1().index();
    size_type ind02 = e.node2().index();
    ...
    // remove reference to current edge in adjacency list
    auto it01 = std::find(nodes_[i2u_[ind01]].edge_inds_.begin(), nodes_[i2u_[ind01]].edge_inds_.end(), index);
    auto it02 = std::find(nodes_[i2u_[ind02]].edge_inds_.begin(), nodes_[i2u_[ind02]].edge_inds_.end(), index);
    nodes_[i2u_[ind01]].edge_inds_.erase(it01);
    nodes_[i2u_[ind02]].edge_inds_.erase(it02);
