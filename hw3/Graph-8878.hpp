...
class Graph {
 ...
 Node add_node(const Point& position,const node_value_type value = node_value_type{})
 {
  ...
  Edge edge(size_type i) const {
    // HW0: YOUR CODE HERE
    edge_type new_edge(eid_[i],this);
     size_type n1,n2;
  ...
  Edge add_edge(const Node& a, const Node& b) {
	...
	//if exists find the corresponding id
		...
		{
			if(connectivity[id1][j].first== id2) {
        new_edge_id = connectivity[id1][j].second.edge_id;
...
*/
//--style_1
//--Duplicate code. Should call remove_edge().
//--START
...
//--design_0
//--Good job calling the other remove_edge function!
//--START
...
//--design_0
//--test_edges and test_nodes run 100% successfully.
//--END
