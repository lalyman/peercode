...
static const unsigned INVALID_SIZETYPE = unsigned(-1);
static const unsigned INVALID_KEY = unsigned(-1);
...
class Graph {
 ...
 public:
  ...
  Graph() : numEdges(0) {
    nodeIds[INVALID_KEY] = INVALID_SIZETYPE;
  }
  ...
  class Node : private totally_ordered<Node> {
   ...
   private:
    ...
    Node(const graph_type* _G, size_type _key) 
      : G(const_cast<graph_type*>(_G)), key(_key) {}
  };
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    edge_value_type& value() {
      return const_cast<edge_value_type&>(value_helper());
    }
   ...
   private:
    ...
    Edge(const graph_type* _G, size_type _key1, size_type _key2) 
      : G(const_cast<graph_type*>(_G)), key1(_key1), key2(_key2) {};

    /**
     * Helper function to return the attribute of an edge. This function should contain the
     *   exactly same code for the two value() functions. Here we write a helper function 
     *   to avoid code duplication which may result in possible difficulty in later code 
     *   management.
     * Complexity: O(log(deg_max))
     */
    const edge_value_type& value_helper() const {
  ...
  class NodeIterator : private totally_ordered<NodeIterator> {
   ...
   private:
    ...
    /** A const graph pointer to the graph this node_iterator points to. */
    const graph_type * G;
    ...
    NodeIterator(const graph_type* _G, size_type _nodeId) 
      : G(_G), nodeId(_nodeId) {}
  };
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator> {
   public:
    ...
    /** 
     * Increase the incident_iterator by 1 (to point to the next neighbor). 
     * @pre: the incident_iteratinor does not point to the end of "edge_lists".
     */
   ...
   private:
    ...
    /** A const graph pointer to the graph this edge_iterator points to. */
    const graph_type * G;
