...
class Graph {
  ...
  */
   ...
   *         where @a node2 are all the nodes neighbor to @a node1
   * @pre 0 <= @a index node1 < @a num_nodes() in the graph
   * @post new num_nodes() == old num_nodes()-1
   * @post new num_edges() == old num_edges() - node1.degree()
   *
   * All the data connected to node1 is erased
   * (included all the edges attached to node1)
   * Also node1 is invalidated
   * Iterators [n_it, last) are invalidated where 
   * n_it  iterator pointing to the @a node1 to be erased
   ...
   * it is valid, otherwise search for the next iterator's valid edge
   * Iterators [e_it, last) are invalidated where 
   * e_it  iterator pointing to the edge to be erased
   * Performs at most O(num_edges()) operation
   */
 ...
 /** This function returns the first valid edge iterator
