...
class Graph {
  ...
  public:
    ...
    const Edge &add_edge(const Node &a, const Node &b, const E &val = E()) {
      ...
      Edge e(a, b, const_cast<E &>(val), num_edges());
      edges_->push_back(e);
      (*nodes_)[a.index()].node_->incident_edges_->insert(e);
      (*nodes_)[b.index()].node_->incident_edges_->insert(e);
  ...
  private:
    ...
    struct InternalNode {
      ...
      InternalNode(Point &pos, V &val, size_type ind)
          : pos_(pos), val_(val), incident_edges_(new std::set<Edge>()), ind_(ind) {
      }
      ...
      std::shared_ptr<std::set<Edge>> incident_edges_;
      size_type ind_;
    };
