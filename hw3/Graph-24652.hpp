...
class Graph {
 ...
 public:
  ...
  size_type remove_node ( const Node & n ){
    ...
    for (auto it = this->node_information_map[old_index]->node->edge_begin(); it != this->node_information_map[old_index]->node->edge_end();){
      ...
      if ((*it).node_2->node_index != old_index){
      ...
      this->edge_map[old_index][old_size] = this->edge_map[old_index][old_index];
      this->edge_map[old_size][old_index] = this->edge_map[old_size][old_size];
      this->edge_map[old_index].erase(old_index);
      this->edge_map[old_size].erase(old_size);
      it = it2;
    }
    }
    ...
    for (auto it = this->node_information_map[old_size]->node->edge_begin(); it != this->node_information_map[old_size]->node->edge_end(); ++it){
      ...
      // Avoid the case where the end node is linked to the current one
      if ((*it).node_2->node_index != old_index ){
