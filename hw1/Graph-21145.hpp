...
class Graph {
 private:
  ...
  // code here. Just use the space if you need it.)
	 ...
	 //STL containers
	 std::vector <proxynodes> N;
	 std::vector <proxyedges> E;
	 std::vector<std::vector<unsigned int>>AdjN;//Adjacent Nodes
...
//
  ...
  class Node:private totally_ordered<Node> {
   public:
	...
	size_type degree() const {
		return graph_->AdjN[index_].size();
	}
   ...
   private:
	...
	Node(const graph_type* graph, size_type id)
		:graph_(const_cast<graph_type*>(graph)), index_(id) {
  ...
  Node add_node(const Point& position, const node_value_type& nodeval =node_value_type()) {
    // HW0: YOUR CODE HERE
	  N.emplace_back(position,nodeval);
	  AdjN.push_back({});
	  ++size_;
	  AdjN.push_back({});
    ...
    //(void) a, (void) b;   // Quiet compiler warning
	  if (!(a==b)) {
		  if (!has_edge(a, b)) {
			  ...
			  AdjN[a.index()].push_back(b.index());//Add b as the adjacent node of a
			  AdjN[b.index()].push_back(a.index());//Add a as the adjacent node of b
			  num_edges_++;
  ...
  class IncidentIterator:private equality_comparable<IncidentIterator> {
   public:
    ...
    // Supply definitions AND SPECIFICATIONS for:
	Edge operator*() const {
		size_type currid = node_->index();
		return Edge(graph_, currid, graph_->AdjN[currid][iid_]);
	}
 ...
 private:
  ...
  //   helper functions, data members, and so forth.
	 struct proxynodes {
		 ...
		 proxynodes(Point P1, node_value_type val1) :P(P1), val(val1) {}
	 };
