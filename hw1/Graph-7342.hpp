...
class Graph {
 ...
 public:
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    bool operator==(const edge_type& e) const {
      return graph_ == e.graph && index_ == e.index_;
     ...
     */
    bool operator<(const edge_type& e) const {
      return graph_ < e.graph_ || (graph_ == e.graph && index_ < e.index_);
    }
  ...
  edge_type add_edge(const node_type& a, const node_type& b) {
    ...
    // s and l are to help organize edges_ and make the code more concise
    size_type s = std::min(a.index_, b.index_);
    size_type l = std::max(a.index_, b.index_);
    // Check if such edge already exists
    for (auto& i : node_edges_map_[s])
      if (edges_[i].node2 == l)
        return Edge(this, i, s, l);
    // Add the new edge to node_edges_map_
    node_edges_map_[s].push_back(num_edges());
    node_edges_map_[l].push_back(num_edges());
    // Add the new internal edge to graph
    edges_.push_back({s, l});

    // Return an edge that associates with the new internal one
    return Edge(this, num_edges(), s, l);
  }
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator> {
   ...
   private:
    ...
    /** Private Constructor */
    IncidentIterator(const graph_type* graph, size_type node1, size_type index)
      : graph_(const_cast<graph_type*>(graph)), node1_(node1), index_(index) {
    }
  };
 ...
 private:
  ...
  std::map<size_type, std::vector<size_type>> node_edges_map_;
};
