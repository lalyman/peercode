 ...
 return n_id;
  ...
  class Edge : private totally_ordered<Edge> {
   public:
     ...
     */
    bool operator<(const Edge& e) const {
      if (min(this->node_1.n_id, this->node_2.n_id) < min(e.node_1.n_id, e.node_2.n_id) )
	return true;
	else if (min(this->node_1.n_id, this->node_2.n_id) > min(e.node_1.n_id, e.node_2.n_id))
	return false;
	else if (max(e.node_1.n_id, e.node_2.n_id) < max(e.node_1.n_id, e.node_2.n_id))
	return true;
	else return false;
    }
  ...
  Edge edge(size_type i) const {
	...
	if (ni.node_position < ni.graph_pointer->adjacency_matrix[ni.node_position][ni.edge_position]) { //to count everything once //
	if (counter == i)
	return Edge(Node(const_cast<Graph*>(ni.graph_pointer), ni.node_position),Node(const_cast<Graph*> (ni.graph_pointer),ni.graph_pointer->adjacency_matrix[ni.node_position][ni.edge_position]));
	else counter++;
	} 
	}
	return Edge();           
  ...
  bool has_edge(const Node& a, const Node& b) const {
	...
	if (a.graph_pointer->adjacency_matrix[a.n_id][i] == b.n_id)
	return true;
	}
	return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
		...
		assert(a.graph_pointer == b.graph_pointer);
		a.graph_pointer->adjacency_matrix[a.n_id].push_back(b.n_id);
		b.graph_pointer->adjacency_matrix[b.n_id].push_back(a.n_id);
                ++num_edges_; 
                return Edge(a, b);
            }
  }
...
// private constructor
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator> {
   public:
    ...
    // Supply definitions AND SPECIFICATIONS for:
    Edge operator*() const {
	return Edge(Node(const_cast<Graph*>(this->graph_pointer), this->node_id),Node(const_cast<Graph*>(this->graph_pointer), this->graph_pointer->adjacency_matrix[node_id][edge_position]));
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator>  {
   public:
    ...
    // Supply definitions AND SPECIFICATIONS for:
     Edge operator*() const {
	return Edge(Node(const_cast<Graph*>(graph_pointer), node_position),Node(const_cast<Graph*>(graph_pointer), graph_pointer->adjacency_matrix[node_position][edge_position]));
	}
