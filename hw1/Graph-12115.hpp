...
class Graph{
 ...
 public:
  ...
  Graph()
    : coordinate_(), adjacency_(), edge_count_(0) {
  }
  ...
  Node add_node(const Point& position, const node_value_type& value = node_value_type()) {
    ...
    new_node.index_ = coordinate_.size();
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    return ((this == n.graph_) && (n.graph_->coordinate_[n.uid_].index_ < coordinate_.size()));
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // HW0: YOUR CODE HERE
    for (unsigned long i = 0; i < adjacency_[a.index()].size(); i++) {
      if (adjacency_[a.index()][i].node_id_ == b.index()) {
	       return true;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
	...
	adjacency_.resize(new_edge.second + 1);
      ...
      adjacency_[a.index()].push_back({new_edge.second});
      adjacency_[b.index()].push_back({new_edge.first});
      edge_count_++;
  ...
  class NodeIterator:private totally_ordered<NodeIterator> {
   public:
    ...
    Node operator*() const {
      return Node(graph_, graph_->coordinate_[uid_].index_);
    }

    NodeIterator& operator++() {
      if (uid_ < graph_->coordinate_.size()) {
	       ++uid_;
      }
      return *this;
    }
