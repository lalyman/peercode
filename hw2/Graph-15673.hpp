...
class Graph {
 ...
 public:
  ...
  class Node:private totally_ordered<Node> {
   ...
   /** Modified verision of node's position */
   Point& position(){
    return graph_->node_vec[uid_].point;
   }
...
* @param[in] Node n from graph
*@pre g.has_node(@a n)
*@pre g.node(i).index()==i 0<=i<num_node()
*@post new num_nodes() = old num_nodes()-1
*@post Edge e with e.node1()==@a n or e.node2()== @a n removed
*@post Edge e with eid of ex_vec is set as -1, meaning removed.
*complexity O(degree()^2)
*Invalidate: node with uid_=n.uid_
              edges incident to node @a n 
             NodeIterator reprenting the removed node @a n    
             EdgeIterator representing the removed edges
             IncidenceIterator representing this removed edge    
*/
  bool remove_node(const Node& n){
  if(!has_node(n)) return false;
  assert(n.uid_<n.graph_->num_nodes());
 // std::cout<<Node(this,n.index()).uid_<<std::endl;
 // std::cout<<n.uid_<<std::endl;
  assert(Node(this,n.index())==n);
  size_type nid = n.uid_;
  size_type end_id = node_vec.size()-1;
  node_vec[nid]=node_vec.back();
 
  node_vec.pop_back(); //remove node from node_vec
  node_vec[nid].uid = nid;
  for(size_type i=0;i<adj_vec[nid].size();i++){
   if(adj_vec[nid][i].adj_uid!=nid){ size_type a_uid = adj_vec[nid][i].adj_uid;
    size_type eid = adj_vec[nid][i].adj_eid;
     for(size_type j = 0;j<adj_vec[a_uid].size();j++){
      if(adj_vec[a_uid][j].adj_uid==nid){
   //    size_type eid = adj_vec[a_uid][j].adj_eid;
     // n.graph_.remove_edge(n,n.graph_->adj_vec[a_uid][j].adj_uid);
        adj_vec[a_uid][j] = adj_vec[a_uid].back();
        adj_vec[a_uid].pop_back();
         ex_vec[eid]=-1;
         }}
          }
          }  //remove adjnode from adj_vec
      adj_vec[nid]=adj_vec.back();
      adj_vec.pop_back();
      for(size_type i = 0; i< adj_vec[nid].size();i++){
         size_type l_uid = adj_vec[nid][i].adj_uid;
         for(size_type j=0;j<adj_vec[l_uid].size();j++){
          if(adj_vec[l_uid][j].adj_uid==end_id) adj_vec[l_uid][j].adj_uid=nid;
            }
           }// change the id and adj_id
  return true; 
}
  ...
  class Edge:private totally_ordered<Edge> {
   public:
    ...
    edge_value_type& value(){
      return graph_->edge_vec[eid_].val;
    };
...
//      return Node(graph_,graph_->edge_vec[eid_].n1);
  ...
  size_type num_edges() const {
   ...
   size_type num=0;
   for(size_type i=0;i<ex_vec.size();i++)
    { if(ex_vec[i]>0) num=num+1;}
  // std::cout<<num;
    return num;
    // return 0;
  }
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!
      if(has_edge(a,b)){
       for(size_type i =0;i<adj_vec[a.uid_].size();i++){ 
       if(adj_vec[a.uid_][i].adj_uid==b.uid_) 
 return Edge(this,adj_vec[a.uid_][i].adj_eid,a.uid_,b.uid_);
     }}
...
*param[in] Node @a a and @a b
*post new num_edge()=old num_edge()-1 if(has_edge(@a a,@a b))
*post edge connecting @a a and @a b is removed
*complexity O(@a a.degree()+ @a b.degree())
Invalidate EdgeIterator representing this edge
           IncidentIterator reprensenting this edge 
*/
  size_type remove_edge(const Node& a, const Node& b){
     if(!has_edge(a,b)) return 0;
     size_type aid = a.uid_;
     size_type bid = b.uid_; 
     for(size_type i=0;i<adj_vec[aid].size();i++){
      if(adj_vec[aid][i].adj_uid==bid){
        size_type eid = adj_vec[aid][i].adj_eid;
        adj_vec[aid][i]=adj_vec[aid].back();
        adj_vec[aid].pop_back();
       //  adj_vec[aid].erase(adj_vec[aid].begin()+i);
        ex_vec[eid]=-1;
       }
} 

     for(size_type i=0;i<adj_vec[bid].size();i++){
      if(adj_vec[bid][i].adj_uid==aid){
         size_type eid = adj_vec[bid][i].adj_eid;
        adj_vec[bid][i]=adj_vec[bid].back();
        adj_vec[bid].pop_back();
       // adj_vec[bid].erase(adj_vec[bid].begin()+i);
       ex_vec[eid]=-1;
     }
     }
 // std::cout<<ex_vec[0]<<std::endl;
   return 1;

    }

  size_type remove_edge(const Edge& e){
   // for(size_type i =0; i<edge_vec.size();i++){
   //  if(edge_vec[i].eid == e.eid_) remove_edge(edge_vec[i].n1,edge_vec[i].n2);
   // }
  ...
  class EdgeIterator:private totally_ordered<EdgeIterator> {
   public:
    ...
    /** return EdgeIterator pointing to next edge or nullptr */
     ...
     edge_id_++;
     while(graph_->ex_vec[edge_id_]<0){edge_id_++;}
     return *this;    
     }
...
/**return EdgeIterator pointing to the first edge */
 ...
 /** return Edge Iterator pointing to the last edge*/
   ...
   size_type i = edge_vec.size()-1;
   while(ex_vec[i]<0){i--;}  
  return EdgeIterator(this,i+1);   
 ...
 private:
  // HW0: YOUR CODE HERE
   ...
   std::vector<int> ex_vec; //a new vector to store information about whether edge exist(1) or not(-1) 
