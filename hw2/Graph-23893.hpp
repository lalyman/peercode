...
class Graph {
 ...
 public:
  ...
  node_type add_node(const Point& position,
    ...
    active_nodes_.push_back(num_nodes()-1);
  ...
  size_type remove_node(const node_type& n) {
    ...
    for (auto iiter = n.edge_begin(); iiter != n.edge_end(); ++iiter) {
      auto e = *iiter;
      remove_edge(e); // O(n.degree) operation
    }
    // Delete this node from the list of nodes
    nodes_.erase(nodes_.begin() + n.index());
    active_nodes_.erase(active_nodes_.begin() + n.index());
    return 1;
  }
  ...
  edge_type add_edge(const node_type& n1, const node_type& n2, 
    ...
    edges_.push_back({n1.index(), n2.index(), num_edges(), value});
    active_edges_.push_back(num_edges()-1);
  ...
  size_type remove_edge(const node_type& n1, const node_type& n2) {
    ...
    for (auto iiter = n1.edge_begin(); iiter != n1.edge_end(); ++iiter) {
      ...
      if (e.node2() == n2)
        edges_.pop_back();
        nodes_[n1.index()].incident_edges.pop_back();
        nodes_[n2.index()].incident_edges.pop_back();
    } 
    return 1;
  }
