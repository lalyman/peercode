...
class Graph {
 ...
 public:
  ...
  class Edge : private totally_ordered<Edge>{
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HERE
      return g_->node(uid1_);
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return g_->node(uid2_);
    }
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator> {
   ...
   private:
    ...
    EdgeIterator(const Graph* graph, size_type uid1) :
      g_(const_cast<Graph*>(graph)), uid1_(uid1), idx_(0) {fix_idx();}
    Graph* g_;
    size_type uid1_, idx_;
  ...
  size_type remove_node(const Node& node){
    ...
    for(auto it = node.edge_begin(); it != node.edge_end(); ++it){
      auto e = *it;
      remove_edge(e.node2(), e.node1());
    }
    // Remove the whole vector for this node in the adjacency list
    adj_.erase(adj_.begin() + node.index());
    // Then remove the node
    size_type i = node.index(); // remember the index
    nodes_.erase(nodes_.begin() + node.index());
    // Reindex the containters
    reindex(i);
    return 0; // TODO: what should I return?
  }
 ...
 private:
  /** Reindexes the internal representation of the graph after deleting a node
   *
   * @param[in] idx The index of the recently deleted Node.
   *
   * After exectution, all nodes or references to nodes with index @a idx or
   * above will have been decremented by 1.
   */
  void reindex(size_type idx){
    for(size_type i = idx; i < nodes_.size(); i++){
      nodes_[i].uid_--;
    }
    for(size_type i = 0; i < adj_.size(); i++){
      for(size_type j = 0; j < adj_.at(i).size(); j++){
        if(adj_.at(i).at(j).uid1_ >= idx){
          adj_.at(i).at(j).uid1_--;
        }
        if(adj_.at(i).at(j).uid2_ >= idx){
          adj_.at(i).at(j).uid2_--;
        }
      }
    }
  }
