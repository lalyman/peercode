...
public:
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      return graph_->coordinate_[uid_];
    }
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE HERE
    coordinate_.push_back(position);
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    return n.uid_ < coordinate_.size();
  }
  ...
  size_type num_edges() const {
    // HW0: YOUR CODE HERE
    int count = 0;
    for (size_type j = 0; j < adjacency_.size(); j++) {
      for (size_type k = 0; k < adjacency_[j].size(); k++) {
	// not counting twice
	if (j < adjacency_[j][k]) {
	  count++;
	}
  ...
  Edge edge(size_type i) const {
	...
	if (j < adjacency_[j][k]) {
	  if (count == i) {
	    return Edge(this, j, adjacency_[j][k]);
	  }
	  count++;
	}
      }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // HW0: YOUR CODE HERE
    return std::find(adjacency_[a.index()].begin(), adjacency_[a.index()].end(), b.index()) != adjacency_[a.index()].end();
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (!has_edge(a, b)) {
      adjacency_[nod1].push_back(nod2);
      adjacency_[nod2].push_back(nod1);
    }
    return Edge(this, nod1, nod2);
  }
