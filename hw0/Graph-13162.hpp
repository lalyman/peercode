 ...
 public:
  ...
  Node node(size_type i) const {
    if (i >= this->nodes_.size()) {
      std::cout << "In Graph::node : Graph has only " << this->nodes_.size() 
                << " nodes, no node number " << i << "!" << std::endl
                << "Return node number " << this->nodes_.size() - 1 
      			<< ". " << std::endl;
  ...
  class Edge {
   public:
    ...
    Edge() : graph_(NULL), id_(size_type(0)) {
    }
  ...
  Edge edge(size_type i) const {
    if (i >= this->edges_.size()) {
      std::cout << "In Graph::node : Graph has only " << this->edges_.size() 
      			<< " edges, no edge number " << i << "!" << std::endl
        		<< "Return edge number " << this->edges_.size() - 1 
        		<< ". " << std::endl;
  ...
  bool has_edge(const Node& a, const Node& b) const {
    for (size_type iter = 0; iter < edges_.size(); ++iter) {
      if ((edges_[iter].v1 == std::min(a.id_, b.id_)) && 
          (edges_[iter].v2 == std::max(a.id_, b.id_))) {
      	return true;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    if (a == b) {
      std::cout<<"In Graph::add_edge : two nodes are equal! "<<std::endl;
      return Edge();
    }
    if (!(this->has_node(a)) || !(this->has_node(b))) {
      std::cout<<"In Graph::add_edge : not all nodes are in the graph! "
      		   <<std::endl;
      return Edge();
    }
