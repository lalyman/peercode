 ...
 public:
  ...
  class Node {
   ...
   private:
    ...
    const graph_type* parentGraph;
    size_type ind;

    // private constructor
    Node(const Graph* g, size_type i) : parentGraph(g), ind(i) {}
  };
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (size_type i =0; i < nEdges; i++){
      if (edge_list[i].node1()==a && edge_list[i].node2() == b){
          return true;
      }
      else if(edge_list[i].node2()==a && edge_list[i].node1() == b){
          return true;
      }
    }
    return false;
