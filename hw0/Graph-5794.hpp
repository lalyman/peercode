...
class Graph {
 ...
 public:
  ...
  Graph() 
    : nodes_(), edges_(), size_(0), esize_(0), idx_(0), eidx_(0){
    // HW0: YOUR CODE HERE
  }
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE HERE
    nodes_.push_back(*new internal_node);
    nodes_.at(size_).position = position;
    nodes_.at(size_).idx = idx_;
    
    ++size_;
    ++idx_;

    (void) position;      // Quiet compiler warning
    return Node(this, idx_-1);        // Invalid node
  }
    ...
    if (has_edge(a, b)){
      for(internal_edge e : edges_){
        if ((e.n1 == a && e.n2 == b) || (e.n1 == b && e.n2 == a)){
          return Edge(this, e.idx);
        }
      }
    }
    edges_.push_back(*new internal_edge);
    edges_.at(esize_).n1 = a;
    edges_.at(esize_).n2 = b;
    edges_.at(esize_).idx = eidx_;
    
    ++esize_;
    ++eidx_;
    (void) a, (void) b;   // Quiet compiler warning
    return Edge(this, eidx_-1);        // Invalid Edge
  }
