 ...
 public:
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      size_type index1 = graph_->edges[uid_].index1;
      return graph_->node(index1);
    }

    /** Return the other node of this Edge */
    Node node2() const {
      size_type index2 = graph_->edges[uid_].index2;
      return graph_->node(index2);
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    for (unsigned i = 0; i < edges.size(); i++) {
      internal_edge e = edges[i];
      node_type node1 = this->node(e.index1);
      node_type node2 = this->node(e.index2);
      if (((node1 == a) && (node2 == b)) || ((node1 == b) && (node2 == a))) {
        return true;
      }
    }
    return false;
